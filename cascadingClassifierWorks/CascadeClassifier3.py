...
neuralNetworkAllFeatures = NeuralNet(
...
for x_test in X_test_Matrix:
    ...
    if confidence >=0.9 and prediction == 0:
        ...
        x_test_comment = x_test[:,15:35]#taking media features
        ...
        if confidence >=1.0 and prediction == 0:
            ...
            x_test_negativity = x_test[:,31:35]#taking media features
            prediction = logReg_negativityfeature.predict(x_test_negativity)[0]
            truth = Y_test_Matrix[i]
            theta = logReg_negativityfeature.coef_[0]
            intercept =  logReg_negativityfeature.intercept_
            confidence = abs(confidenceLogRegression(theta, x_test_negativity,intercept)-0)
            if confidence >=0.7 and prediction == 0:
                total = total + 1
                if prediction == truth:
                    correct = correct + 1
            elif confidence >=2.0 and prediction == 1:
                total = total + 1
                if prediction == truth:
                    correct = correct + 1
            else:
                x_test_all = x_test[:,0:35]#taking media features
                prediction = logReg_allfeature.predict(x_test_all)[0]
                truth = Y_test_Matrix[i]
                theta = logReg_allfeature.coef_[0]
                intercept =  logReg_allfeature.intercept_
                confidence = abs(confidenceLogRegression(theta, x_test_all,intercept)-0)
                if confidence >=1.9 and prediction == 0:
                    total = total + 1
                    if prediction == truth:
                        correct = correct + 1
                elif confidence >=6 and prediction == 1:
                    total = total + 1
                    if prediction == truth:
                        correct = correct + 1   
    i=i+1
