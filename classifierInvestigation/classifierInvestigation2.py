...
def incrementalVersionWithMemory():
    mediaIdLimits = [100000]
    ...
    for mediaIdLimit in mediaIdLimits:
        ...
        arrayOfData = np.zeros((mediaIdLimit,6),dtype=np.float)
        for mediaid in range(0,mediaIdLimit):
            ...
            arrayOfData[mediaid][0] = followerCount
            arrayOfData[mediaid][1] = followingCount
            arrayOfData[mediaid][2] = likeCount
            arrayOfData[mediaid][3] = loopCount
            arrayOfData[mediaid][4] = postCount
            arrayOfData[mediaid][5] = logisticValue
        break
    ...
    for mediaIdLimit in mediaIdLimits:
        ...
        for mediaid in range(0,mediaIdLimit):
            ...
            loopCount = random.choice(loopCountList)
            postCount = random.choice(postCountList)
            if followerCount != arrayOfData[mediaid][0]:
                arrayOfData[mediaid][5] = arrayOfData[mediaid][5] - theta[0]*arrayOfData[mediaid][0] + followerCount*theta[0]
                arrayOfData[mediaid][5] = followerCount
            finalValue = float(arrayOfData[mediaid][5]) + intercept
            if finalValue > 0.0:
                prediction = 1
            else:
                prediction = 0
...
#check with incremental version with memory instead of database
incrementalVersionWithMemory()
